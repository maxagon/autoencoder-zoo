...
class ImgDimReduceMini(ae.AEBase):
    def __init__(self, in_out_dim, ae_depth, ae_dim, ae_lat_dim, unet_depth, unet_dim, img_size):
        ...
        self.down_blocks = ae_factory.unet_downscale_blocks(img_size=img_size, dims=ae_dim, depth=ae_depth)
        self.up_blocks = ae_factory.unet_upscale_blocks(img_size=img_size, dims=r_ae_dim, depth=r_ae_depth)
        self.from_img = ae_factory.make_dim_convert(img_size=img_size, in_dim=in_out_dim, out_dim=ae_dim[0])
        self.to_lat = ae_factory.make_dim_convert(img_size=img_size, in_dim=ae_dim[-1], out_dim=ae_lat_dim)
        self.from_lat = ae_factory.make_dim_convert(img_size=img_size, in_dim=ae_lat_dim, out_dim=r_ae_dim[0])
        self.to_img = ae_factory.make_dim_convert(img_size=img_size, in_dim=r_ae_dim[-1], out_dim=in_out_dim)
