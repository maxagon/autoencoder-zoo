...
class ConvMemory(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.register_parameter("memory", nn.Parameter(torch.rand(size=[in_channels * kernel_size * kernel_size, out_channels])))
    def forward(self, x : torch.Tensor):
        b, c, h, w = x.shape
        out = x
        out = torch.nn.functional.unfold(out, kernel_size=self.kernel_size, padding=self.kernel_size//2)
...
class PathMemory(nn.Module):
    def __init__(self, path_size, in_channels, out_channels, head_dim, n_heads):
        ...
        self.heads = n_heads
        self.linear = ff.Linear(in_dim=dim_in, out_dim=path_size * path_size * head_dim)
        self.register_parameter("memory", nn.Parameter((torch.rand(size=[n_heads, head_dim, head_out]) * 2.0 - 1.0)))

    def forward(self, x):
        b, c, h, w = x.shape
        ph = h // self.path_size
        out = x
        out = rearrange(out, "b c (h p1) (w p2) -> b (h w) (p1 p2 c)", p1=self.path_size, p2=self.path_size)
        out = self.linear(out)
        out = rearrange(out, "b p (c m)-> b m c p", m=self.heads)
        out = nn.functional.softmax(out, dim=2)
        out = torch.einsum("nki,bnkl->bnil", self.memory, out)
        out = rearrange(out, "b m (p1 p2 c) (h w) -> b (m c) (h p1) (w p2)", m=self.heads, p1=self.path_size, p2=self.path_size, h=ph)
        return out

class KeyChannelwiseMemory(nn.Module):
    def __init__(self, in_dim, memory_dim, out_dim):
        super().__init__()
        self.register_parameter("key", nn.Parameter((torch.rand(size=[in_dim, memory_dim]))))
        self.register_parameter("memory", nn.Parameter(torch.rand(size=[memory_dim, out_dim]) * 2.0 - 1.0))
...
class KeyConvMemoryMultiHead(nn.Module):
    def __init__(self, in_dim, n_heads, key_dim, memory_dim, head_dim, out_dim, kernel_size):
        super().__init__()
        self.n_heads = n_heads
        self.head_out = out_dim // n_heads
        self.kernel_size = kernel_size
