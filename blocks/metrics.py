...
class SSIM(nn.Module):
    ...
    def forward(self, input : torch.Tensor, grountruth : torch.Tensor):
        ...
        with torch.no_grad():
            return tm.functional.structural_similarity_index_measure(input, grountruth.to(input.dtype), data_range=2.0)
...
class PSNR(nn.Module):
    ...
    def forward(self, input : torch.Tensor, grountruth : torch.Tensor):
        assert(input.device == grountruth.device)
        with torch.no_grad():
            return tm.functional.peak_signal_noise_ratio(input, grountruth.to(input.dtype), data_range=2.0)
