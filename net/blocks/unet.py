...
class UNetFactory:
    ...
    def unet_downscale_blocks(self, dims, depth, heads):
        ...
        if depth[0] != 0:
            modules.append(
                self.make_feedforward(dim=dims[0], n_layers=depth[0], heads=heads[0])
            )
        for i in range(1, len(dims)):
            ...
            cur_dim = next_dim
            blocks.append(
                self.make_feedforward(dim=cur_dim, n_layers=depth[i], heads=heads[i])
            )
...
class UnetReconstruction(nn.Module):
    def __init__(
        ...
        self.down_blocks = factory.unet_downscale_blocks(
            dims=dim, depth=depth, heads=heads
        )
        self.up_blocks = factory.unet_upscale_blocks(
